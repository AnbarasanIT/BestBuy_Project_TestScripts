...
/**
 *  Unit Test class for ExpiredAssets Job
 *  @author Lakshmi.Valluripalli
 */
public class ExpiredAssetsJobTest {
	
	private final static Logger logger = Logger.getLogger(ExpiredAssetsJobTest.class);
	...
	/**
	 * Tests the Execute Method of ExpiredAssetsJob
	 * @throws JobExecutionException
	 */
	@Test
	public void testJobExecute() throws JobExecutionException{
		/*try{
			ExpiredAssetsJob job = new ExpiredAssetsJob();
			List<Asset> assets = new ArrayList<Asset>();
			Asset asset = BaseData.getAssets();
			assets.add(asset);
			...
			doAnswer(new Answer() {
		 	    ...
		 	    }).when(assetsService).update(assets);
			job.execute(ctx);
		}catch(Exception e){
